...
Run `npm run record:manual-result -- --check <check-id>=pass` to update the structured result after each manual check; the command re-runs `npm run verify:manual-result` after writing.
