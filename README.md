...
- `src-tauri/target/release/bundle/dmg/Hosts Switch_0.1.15_aarch64.dmg`
...
Use `docs/release/manual-validation-v0.1.15.md` and `docs/release/manual-validation-v0.1.15.result.json` to record these checks. Run `npm run verify:manual-validation` to confirm the release checklist and manual validation template stay in sync.
...
Release notes are maintained in `docs/release/release-notes-v0.1.15.md`; `npm run verify:release-notes` checks that the Chinese version notes, asset name, manual-validation warning, and SHA-256 line stay ready for publication.
...
- Open the packaged `.app` or install from the verified release asset `manual-validation-artifacts/v0.1.15/Hosts.Switch_0.1.15_aarch64.dmg`.
...
The release workflow publishes `Hosts.Switch_0.1.15_aarch64.dmg` as the downloadable asset. Local Tauri builds still produce `Hosts Switch_0.1.15_aarch64.dmg`.
GitHub release notes are rendered from `docs/release/release-notes-v0.1.15.md`, with the workflow substituting the final DMG SHA-256 before publishing.
