...
这是 Hosts Switch 的 macOS 预发布构建，用于验证 Tauri + React 版 hosts 切换工具。`v0.1.19` 基于 `v0.1.18` 修复安装后看不到入口的问题，并继续保留空 `/etc/hosts` 场景下的防误写保护。

本版本重点：

- 修复 macOS 顶部状态栏没有可见 app 图标的问题：状态栏入口现在显式使用打包内置 app icon，安装后可从菜单栏打开切换菜单。
- 修复双击打开本地 app 后看不到主界面的问题：首次启动会直接显示编辑器窗口；关闭窗口后仍会隐藏到状态栏，状态栏图标可重新打开编辑器。
- 空 `/etc/hosts` 的提示改为明确要求先恢复安全基线：`Current /etc/hosts is empty. Restore a safe /etc/hosts baseline before applying changes.`，避免误以为可以直接确认后继续写入。
...
- GitHub Release notes 继续从仓库内受校验的 `docs/release/release-notes-v0.1.19.md` 渲染发布，减少发布页面和源码记录不一致。
...
- 如果当前 `/etc/hosts` 是 0 bytes，请先按 `npm run print:hosts-recovery` 输出的只读指南手动恢复默认 hosts 内容，再测试 Apply 或状态栏切换。

## English Summary

Automated prerelease build for v0.1.19.

Artifact:

- Hosts.Switch_0.1.19_aarch64.dmg
- SHA-256: `a264a75df7f595c7e20af207ccf087cc1f1d858c4ed4c6f593ba3a4f181eb5d5`
